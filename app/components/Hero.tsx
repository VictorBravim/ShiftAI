import { FC } from "react";

const Hero: FC = () => {
    return (
        <div className="hero min-h-screen bg-black text-white flex items-center justify-center">
            <div className="absolute top-0 left-0 w-full">
                <nav className="flex justify-between items-center p-12 bg-transparent">
                    <div className="rounded-full">
                        <img src="app/assets/Logo.png" alt="Logo" className="w-24 h-24 object-contain"/>
                    </div>
                    <div className="w-full flex justify-between items-center space-x-12 bg-gradient-to-r from-white to-blue p-7 rounded-full mx-6">
                        <div className="flex space-x-12">
                            <button className="text-dark font-bold">SOLUTIONS</button>
                            <button className="text-dark font-bold">INTEGRATIONS</button>
                            <button className="text-dark font-bold">PRICING</button>
                        </div>
                        <div className="flex space-x-12">
                            <button className="text-white font-bold">SIGN IN</button>
                        </div>
                    </div>
                    <button className="px-8 py-6 bg-blue text-white font-bold rounded-full whitespace-nowrap">
                        REQUEST SET-UP
                    </button>
                </nav>
            </div>
            <div className="w-full h-screen flex items-center justify-center p-4">
                <div className="bg-dark flex flex-row items-center justify-center rounded-[50px] w-full h-full p-4">
                    <div className="bg-dark flex items-end w-[70%] h-full p-10 pt-64">
                        <img src="app/assets/Hero.png" alt="image" className="w-full h-full" />
                    </div>
                    <div className="bg-dark flex justify-end w-1/3 h-full items-end">
                        <button className="px-9 py-11 bg-white text-dark font-bold rounded-full">
                            Scroll
                        </button>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Hero;